...
async function login(request, env) {
  ...
  return json({ ok: true }, 200, request, { 'Set-Cookie': `fan_room_admin=${token}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=28800` });
}
...
async function signedIn(request, env) {
  const value = (request.headers.get('Cookie') || '').match(/(?:^|; )fan_room_admin=([^;]+)/)?.[1];
