...
function adminPage() {
  return new Response(`<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>FAN ROOM 管理</title><style>body{max-width:900px;margin:40px auto;padding:0 18px;background:#07080d;color:#eee;font-family:system-ui}input,textarea,select,button{box-sizing:border-box;width:100%;margin:6px 0;padding:10px;background:#141827;color:#fff;border:1px solid #ffffff33}button{cursor:pointer;color:#d7b777}.item{margin:18px 0;padding:18px;border:1px solid #ffffff22;white-space:pre-wrap}.hidden{display:none}</style><h1>FAN ROOM 管理</h1><section id="login-panel"><input id="pw" type="password" placeholder="管理パスワード"><button onclick="login()">ログイン</button><p id="err"></p></section><section id="app" class="hidden"><button onclick="logout()">ログアウト</button><div id="list"></div></section><script>const $=s=>document.querySelector(s),TOKEN='fan_room_admin_token';const auth=()=>{const t=sessionStorage.getItem(TOKEN);return t?{Authorization:'Bearer '+t}:{}};function loginBox(ok){$('#login-panel').classList.toggle('hidden',ok);$('#app').classList.toggle('hidden',!ok)}async function login(){const r=await fetch('/admin/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({password:$('#pw').value})}),d=await r.json();if(!r.ok||!d.token){$('#err').textContent='パスワードを確認してください';return}sessionStorage.setItem(TOKEN,d.token);await load()}async function load(){const r=await fetch('/admin/messages',{headers:auth()});if(!r.ok){loginBox(false);return}const d=await r.json();loginBox(true);$('#list').innerHTML=d.items.map(i=>'<article class="item"><small>'+i.display_name+' / '+i.category+' / '+i.created_at+'</small><h3>'+esc(i.nickname)+'</h3><p>'+esc(i.body)+'</p><label>状態</label><select id="s-'+i.id+'"><option value="pending" '+(i.status==='pending'?'selected':'')+'>確認中</option><option value="approved" '+(i.status==='approved'?'selected':'')+'>承認</option><option value="archived" '+(i.status==='archived'?'selected':'')+'>保管</option></select><label>返信</label><textarea id="r-'+i.id+'">'+esc(i.artist_reply||'')+'</textarea><label><input id="p-'+i.id+'" type="checkbox" '+(i.reply_published?'checked':'')+'>公開する</label><button onclick="save(\''+i.id+'\')">保存</button></article>').join('')}async function save(id){await fetch('/admin/messages/'+id,{method:'PATCH',headers:{'content-type':'application/json',...auth()},body:JSON.stringify({status:$('#s-'+id).value,reply:$('#r-'+id).value,publish:$('#p-'+id).checked})});load()}async function logout(){sessionStorage.removeItem(TOKEN);await fetch('/admin/logout',{method:'POST'});location.reload()}function esc(v){return String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}load()</script>`,{headers:{'Content-Type':'text/html; charset=utf-8'}});
}
