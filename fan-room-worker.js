...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname.startsWith('/admin/messages/') && request.method === 'DELETE') return deleteMessage(request, env, url.pathname.split('/').pop());
...
async function deleteMessage(request, env, id) {
  if (!await signedIn(request, env)) return json({ error: 'Unauthorized' }, 401, request);
  await env.DB.prepare('DELETE FROM fan_messages WHERE id = ?').bind(id).run();
  return json({ ok: true }, 200, request);
}
...
async function updateArtForm(request, env, id) {
  if (!await signedIn(request, env)) return redirect('/admin?error=session');
  const d=await request.formData();
  if(d.get('delete')==='1'){
    await env.DB.prepare('DELETE FROM fan_art WHERE id=?').bind(id).run();
    return redirect('/admin/art?deleted=1');
  }
  const status=['pending','approved','archived'].includes(d.get('status'))?d.get('status'):'pending';
  await env.DB.prepare('UPDATE fan_art SET status=?, updated_at=? WHERE id=?').bind(status,new Date().toISOString(),id).run();
  return redirect('/admin/art?saved=1');
}
async function adminArtPage(request, env, url) {
  ...
  const items=results.map(x=>`<article><small>${escHtml(x.created_at)}</small><h3>${escHtml(x.nickname)}</h3><div class="art-preview"><blockquote class="twitter-tweet" data-theme="dark" data-dnt="true"><a href="${escHtml(x.post_url)}"></a></blockquote></div><a class="x-open" href="${escHtml(x.post_url)}" target="_blank" rel="noreferrer">Xで元の投稿を開く ↗</a><form method="post" action="/admin/art/${encodeURIComponent(x.id)}"><select name="status"><option value="pending" ${x.status==='pending'?'selected':''}>確認中</option><option value="approved" ${x.status==='approved'?'selected':''}>掲載する</option><option value="archived" ${x.status==='archived'?'selected':''}>非公開</option></select><button>保存</button><button type="submit" name="delete" value="1" onclick="return confirm('このファンアートを完全に削除します。元に戻せません。')">完全に削除</button></form></article>`).join('')||'<p>まだ投稿はありません。</p>';
  return new Response(`<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>FAN ART 管理</title><style>body{max-width:760px;margin:24px auto;padding:0 16px;background:#07080d;color:#eee;font-family:system-ui}a{color:#d7b777}.x-open{display:block;margin:10px 0 16px;text-align:right}.art-preview{max-width:550px;margin:14px auto;overflow:hidden}.twitter-tweet{margin:0 auto!important}select,button{box-sizing:border-box;width:100%;margin:8px 0;padding:14px;background:#141827;color:#fff;border:1px solid #ffffff33;font-size:16px}button{color:#d7b777}article{padding:18px;margin:16px 0;border:1px solid #ffffff22;word-break:break-all}@media(max-width:560px){body{padding:0 12px}article{padding:14px}}</style><h1>FAN ART 管理</h1><p><a href="/admin">← メッセージ管理</a></p>${url.searchParams.get('saved')?'<p>保存しました。</p>':''}${url.searchParams.get('deleted')?'<p>ファンアートを完全に削除しました。</p>':''}${items}<script async src="https://platform.x.com/widgets.js" charset="utf-8"></script>`,{headers:{'Content-Type':'text/html; charset=utf-8'}});
}
